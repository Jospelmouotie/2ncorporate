      ...
      <div class="mt-32 border-t border-slate-100 pt-20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-16">
          <div class="lg:col-span-4 space-y-8">
            ...
            <div v-if="user" class="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-2xl">
            ...
            <div v-else class="p-8 bg-orange-50 rounded-[2.5rem] border-2 border-orange-100 text-center">
              ...
              <router-link to="/login" class="inline-block bg-slate-900 text-white px-8 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest">Se connecter</router-link>
            </div>
          </div>
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div class="flex text-orange-400 mt-1"><Star v-for="i in 5" :key="i" :class="i <= review.rating ? 'fill-current' : 'text-slate-200'" class="w-3 h-3" /></div>
                            </div>
                        </div>
                    </div>
...
// REMPLACER AuthStore par une vérification simple
// On suppose que l'utilisateur est passé via window.Laravel.user (standard Laravel)
...
const submitReview = async () => {
  ...
  try {
    await axios.post(`/api/products/${product.value.id}/reviews`, reviewData)
    modal.open("Merci ! Votre avis a été enregistré.", null, 'success')
    fetchProduct()
    reviewData.comment = ''
...
const allImages = computed(() => {
  ...
  const imgs = [product.value.image_url, ...(product.value.images || []).map(i => typeof i === 'object' ? i.url : i)]
  return [...new Set(imgs)].filter(Boolean)
})

const checkMobile = () => { isMobile.value = window.innerWidth < 1024 }
onMounted(() => { fetchProduct(); checkMobile(); window.addEventListener('resize', checkMobile) })
